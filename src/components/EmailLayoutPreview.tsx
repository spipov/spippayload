  ...
  const generatePreview = (): void => {
    ...
    try {
      ...
      const headerSettings = {
        enabled: (data as any).headerEnabled,
        layout: (data as any).headerLayout,
        backgroundColor: (data as any).headerBackgroundColor,
        textColor: (data as any).headerTextColor,
        customHtml: (data as any).headerCustomHtml,
        padding: {
          top: (data as any).headerPaddingTop,
          bottom: (data as any).headerPaddingBottom,
          left: (data as any).headerPaddingLeft,
          right: (data as any).headerPaddingRight,
        },
        border: {
          topEnabled: (data as any).headerBorder?.headerTopBorderEnabled,
          bottomEnabled: (data as any).headerBorder?.headerBottomBorderEnabled,
          width: (data as any).headerBorder?.headerBorderWidth,
          color: (data as any).headerBorder?.headerBorderColor,
          style: (data as any).headerBorder?.headerBorderStyle,
        },
        logoSettings: (data as any).logoSettings,
      }
      const footerSettings = {
        enabled: (data as any).footerEnabled,
        layout: (data as any).footerLayout,
        backgroundColor: (data as any).footerBackgroundColor,
        textColor: (data as any).footerTextColor,
        customHtml: (data as any).footerCustomHtml,
        padding: {
          top: (data as any).footerPaddingTop,
          bottom: (data as any).footerPaddingBottom,
          left: (data as any).footerPaddingLeft,
          right: (data as any).footerPaddingRight,
        },
        border: {
          topEnabled: (data as any).footerBorder?.footerTopBorderEnabled,
          bottomEnabled: (data as any).footerBorder?.footerBottomBorderEnabled,
          width: (data as any).footerBorder?.footerBorderWidth,
          color: (data as any).footerBorder?.footerBorderColor,
          style: (data as any).footerBorder?.footerBorderStyle,
        },
      ...
      const generateHeaderHTML = () => {
        ...
        const headerStyle = {
          ...
          borderTop: headerSettings.border?.topEnabled
            ? `${headerSettings.border?.width || 1}px ${headerSettings.border?.style || 'solid'} ${headerSettings.border?.color || '#e9ecef'}`
            : 'none',
          borderBottom: headerSettings.border?.bottomEnabled
            ? `${headerSettings.border?.width || 1}px ${headerSettings.border?.style || 'solid'} ${headerSettings.border?.color || '#e9ecef'}`
            : 'none',
        }
        ...
        return `<div style="${Object.entries(headerStyle)
      ...
      const generateFooterHTML = () => {
        ...
        const footerStyle = {
          ...
          textAlign: 'center' as const,
          fontSize: '14px',
          borderTop: footerSettings.border?.topEnabled
            ? `${footerSettings.border?.width || 1}px ${footerSettings.border?.style || 'solid'} ${footerSettings.border?.color || '#e9ecef'}`
            : 'none',
          borderBottom: footerSettings.border?.bottomEnabled
            ? `${footerSettings.border?.width || 1}px ${footerSettings.border?.style || 'solid'} ${footerSettings.border?.color || '#e9ecef'}`
            : 'none',
        }
        ...
        if (footerSettings.layout === 'custom') {
          ...
          if (sections.showSocialLinks !== false) {
            parts.push(
              '<div style="margin-bottom: 10px;"><a href="#" style="margin: 0 5px;">Facebook</a> | <a href="#" style="margin: 0 5px;">Twitter</a> | <a href="#" style="margin: 0 5px;">LinkedIn</a></div>',
            )
          }

          if (sections.showContactInfo !== false) {
            parts.push(
              '<div style="margin-bottom: 10px;">{{company_address}} | {{support_email}} | {{support_phone}}</div>',
            )
          }

          if (sections.showLegalLinks !== false) {
            parts.push(
              '<div style="margin-bottom: 10px;"><a href="#">Unsubscribe</a> | <a href="#">Preferences</a> | <a href="#">Privacy Policy</a></div>',
            )
          }
        ...
        return `<div style="${Object.entries(footerStyle)
          .map(([key, value]) => `${key.replace(/([A-Z])/g, '-$1').toLowerCase()}: ${value}`)
          .join('; ')}">${content}</div>`
      }
      ...
      setPreviewHtml(
        '<div style="color: red; padding: 20px; text-align: center;">Error generating preview</div>',
      )
    } finally {
      setLoading(false)
    }
